...
export async function generateReport(inspection) {
  ...
  if (inspection.declarations?.length > 0) {
    ...
    doc.setFontSize(13);
    doc.setTextColor(15, 23, 42);
    doc.text('Extracted Declarations', 14, y);
    y += 2;
    doc.line(14, y, 60, y);
    y += 6;
  ...
  doc.setFont('helvetica', 'italic');
  doc.text(
    'Disclaimer: This report is generated by an AI-assisted tool for preliminary screening purposes only. Final compliance determination must be made by an authorized Legal Metrology Officer.',
    14, disclaimerY, { maxWidth: pageW - 28 }
  );

  /* ── Save ── */
  doc.save(`PackSure_Report_${inspection.id}.pdf`);
}
